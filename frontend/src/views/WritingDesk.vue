  ...
  if (selectedChapter.value?.versions && Array.isArray(selectedChapter.value.versions)) {
    ...
    const convertedVersions = selectedChapter.value.versions.map((versionString, index) => {
      ...
      try {
        ...
        logger.debug(`版本 ${index} JSON解析失败，使用原始字符串:`, error)
        return {
    ...
    logger.debug('转换后的版本对象:', convertedVersions)
    return convertedVersions
  }

  logger.debug('没有可用版本，selectedChapter:', selectedChapter.value)
  return []
})
...
const fetchChapterStatus = async () => {
  ...
  try {
    ...
    logger.debug('Chapter status polled and updated.')
  } catch (error) {
    logger.error('轮询章节状态失败:', error)
    // 在这里可以决定是否要通知用户轮询失败
  }
}
