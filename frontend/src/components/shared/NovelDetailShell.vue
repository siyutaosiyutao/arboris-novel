        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              <span class="hidden sm:inline">{{ isExporting ? '导出中...' : '导出全文' }}</span>
              <span class="sm:hidden">{{ isExporting ? '...' : '导出' }}</span>
            </button>
            <button
...
const exportAllChapters = async () => {
  ...
  try {
    ...
    // 使用 fetch 下载文件
    const response = await fetch(`/api/writer/novels/${projectId}/export/all-chapters`, {
      method: 'GET',
      headers: {
